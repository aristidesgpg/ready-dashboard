...
export class DashboardComponent implements OnInit, OnDestroy
{
  ...
  ngOnInit(): void
  {
    ...
    var getAnalyticsReports = this.authService.vfiSandboxApp.functions().httpsCallable('GetAnalyticsReport');
    getAnalyticsReports({ projectId: "123456"})
      .then((result) => {
        let conversion = result.data.conversion;
        this.conversion = conversion;
        let last30DaysDailyUsers = result.data.daily_user_for_last_30_days
        this.last30DaysDailyUsersSeries = [
          {
            name: 'Players',
            data: last30DaysDailyUsers
          }
        ]
  ...
  private _prepareChartData(): void
  {
    ...
    this.chartConversions = {
      ...
      series : this.last30DaysDailyUsersSeries,
      stroke : {
        curve: 'smooth'
      },
      tooltip: {
